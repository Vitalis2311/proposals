...
function renderTablesFromJson(data) {
  ...
  data.tables.forEach((section) => {
    ...
    rows.forEach(r => {
      const dataKeys = Object.keys(r).filter(k => !['highlight', 'color'].includes(k));
    ...
    options: {
      ...
      scales: {
        y: {
          ...
          ticks: {
            callback: v => {
              if (rawValues?.some(val => typeof val === 'string' && val.includes('%'))) {
                return `${v.toFixed(1)}%`;
              }
...
$(document).ready(async () => {
  const json = {
    ...
    "tables": [
      {
        "rows": [
          { "": ["Conv 30 Yr Fixed", "FHA 30 Yr Fixed", "VA 30 Yr Fixed"], "highlight": "#4c514f", "color": "#fff" },
      ...
      },
      {
        "rows": [
          ...
          { "Interest Saved over 5 Years": ["($34,824.00)", "($121,234.00)", "($51,357.11)"], "highlight": true },
    ...
    "hideTablesRows": ["APR", "Interest Saved over Life of Loan"],
    "hideCurrentRows": ["Equity", "Appraised Value"]
  };
