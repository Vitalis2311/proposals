...
$(document).ready(async () => {
  const data = {
    ...
    "hideTablesRows": [],
    "hideCurrentRows": ["HOA", "Hazard Insurance"],
    "showPowerRefinanceBenefits": true,
    "showCurrentLoanSection": true
  };

  if (typeof data !== 'object') { console.error('❌ JSON не визначено або неправильний формат.'); return; }

  const $container = $('<div>').attr('id', 'container').addClass('container');
  $('body').append($container);

  $container.append(buildHeaderContainer(data));
  $container.append(buildGreetingContainer(data));
  $container.append(buildMainContent(data));
